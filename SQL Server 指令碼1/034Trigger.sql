--如果訂單商品訂購數量大於庫存數量或售價大於定價，則訂購失敗
--若訂購成功，則扣除訂購的庫存量
--客戶在下訂單時，會動用到三張資料表-訂貨主檔、訂貨明細、產品資料

--假設訂貨主檔新增成功,訂貨明細新增成功,但某一產品訂購數量大於庫存量(需探討的議題)

alter trigger checkOrderQtyPrice on 訂貨明細
after insert
as
begin
	--1.如果訂單商品訂購數量大於庫存數量或售價大於定價，則訂購失敗
	--2.若訂購成功，則扣除訂購的庫存量
	...
	if @OrderProductPrice>@ProductPrice   --如果訂單商品訂購數量大於庫存數量或售價大於定價
		rollback  
